...
The result of the expression `combined(toUpper, " hello")` is to apply the `toUpper` function defined earlier in the header to the `msg` parameter value `hello`. The supplied second argument `hello` overrides the `msg="universe"` default value, so the result is `HELLO WORLD`.
